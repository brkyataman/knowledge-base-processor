...
def save_given_articles(articles):
    for article_x in articles:
        ...
        try:
            with open("microorganisms/" + pubmed_id + '.txt', 'w') as file:
...
def run(query, num_of_articles, the_fetch_size):
    ...
    registered_ids = [f.replace('microorganisms\\', '').replace('.txt', '') for f in glob.glob("microorganisms/*.txt")]
...
fetch_size = 20
run("microorganisms", 400000, fetch_size)
